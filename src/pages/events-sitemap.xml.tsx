...
function generateSiteMap(events: IEventSiteMap[]) {
  ...
  <url>
  <loc>${process.env.NEXT_PUBLIC_SITE_URL}su-kien</loc>
  <lastmod>${lastmod}</lastmod>
  </url>

  <url>
  <loc>${process.env.NEXT_PUBLIC_SITE_URL}end</loc>
  <lastmod>${lastmod}</lastmod>
  </url>
  ...
  ${events
    ?.map(
      (event) => `<url>
  <loc>${process.env.NEXT_PUBLIC_SITE_URL}su-kien/${event?.slug}</loc>
