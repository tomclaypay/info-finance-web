...
import LatestKnowledgeItem from '@app/components/knowledge/LatestKnowledge-item'
...
const KnowledgeDetail = ({ dataArticle, dataRelateArticles, dataLatestArticles }: NewsDetailProps) => {
  ...
  return (
    <>
      <Head>
        ...
        <link
          rel="canonical"
          href={`${locale === 'vi' ? domain.vi + 'kien-thuc' : domain.en + 'knowledge'}/${
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: `
              ...
              "mainEntityOfPage": {
                "@type": "WebPage",
                "@id": "${locale === 'vi' ? domain.vi + 'kien-thuc' : domain.en + 'knowledge'}/${
              dataArticle?.articles?.data?.[0]?.attributes?.slug
            }"
              },
            ...
            __html: `{
              ...
              "itemListElement": "[
                ...
                },
                {
                  ...
                  "position": 2,
                  "name": "Kiến thức",
                  "item": "https://infofinance.com/kien-thuc"
                },
                {
                  "@type": "ListItem",
                  "position": 3,
                  "name": "${dataArticle?.articles?.data?.[0]?.attributes?.seoTitle?.trim()}",
                  "item": "${locale === 'vi' ? domain.vi + 'kien-thuc' : domain.en + 'knowledge'}/${
              dataArticle?.articles?.data?.[0]?.attributes?.slug
            }"
                }
              ]"
      ...
      >
              ...
              {dataArticle && (
                <>
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <NextLink
                      href={{
                        pathname: locale === 'vi' ? '/kien-thuc' : '/knowledge',
                        query: {
                    ...
                    {dataRelateArticles &&
                      (isMobile
                        ? dataRelateArticles.articles.data
                            ...
                            ?.map((item: any) => (
                              <>
                                <LatestKnowledgeItem key={item.id} data={item} image={false} />
        ...
        {dataLatestArticles && <ProviderCategoryAll dataArticlesByCategoryAll={dataLatestArticles} />}
      </Box>
    </>
  )
}

KnowledgeDetail.getLayout = (page: any) => <MainLayout>{page}</MainLayout>

export default KnowledgeDetail

export async function getServerSideProps(context: any) {
  const { locale } = context
  const { knowledgeSlug } = context.query
  try {
